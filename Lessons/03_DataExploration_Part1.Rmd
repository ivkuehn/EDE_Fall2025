...
> Answer: In some cases there is no output in the console, for packages like tidyverse that contain other packages, the other packages are shown as attached, and package conflicts can also appear in the output.
...
# NOTE: Absolute file path (not recommended)
...
```{r}
# Can use read.csv(home('')) and in between the quotations use tab to autocomplete the path
```
...
> ANSWER: You can find this file in the Metadata folder within the Data folder of the cloned repository. The placement and information in the file relates to best practices for reproducible data analysis because not only does it share what information the data has, but also the specifications of how the data is structured, where the data was found, and when the data was collected. Providing the user with the greatest understanding of how the data was stored and organized, along with the source of data being used is the best way to ensure accurate reproducibility.
...
```{r Exploring data}
#View(USGS.flow.data) # NOTE: need to comment out View command when knitting
# Alternate option: click on data frame in Environment tab

class(USGS.flow.data)
colnames(USGS.flow.data) # could also just use names(USGS.flow.data) and it would do the same
...
str(USGS.flow.data) # glimpse() should do the same thing with slightly different formatting
dim(USGS.flow.data)
...
class(USGS.flow.data$datetime) # want this to actually be a date/time object
...
> Answer: From what I see, blank cells are still left blank, or they are filled with 'NA'.
...
```{r Finding NAs}
summary(USGS.flow.data$discharge.mean) # provides summary stats for specific columns
...
> Answer: In the case of calculating summary stats for an entire data set, too many NAs can cause errors, so removing NAs would be favorable. (Like in the case of calculating the mean for this entire data set, the presence of NAs makes the return mean for the dataset NA). In other cases where NAs may be part of the statistic, or less present and therefore more valuable, retaining NAs would be favorable as it's contributing to the statistic. Also, omitting rows that contain an NA entry may end up deleting other present data in that row that cause other statistics to change (ex. many of the gauge height entries had NA values, but valid values for discharge, so deleting the rows of NAs gets rid of discharge values that skew stats later on.)
...
#NOTE: formatting the above line using %b will return Sept instead of September
#format(today, format = "")
#format(today, format = "")
#format(today, format = "")
...
write.csv(
  ...
  row.names=FALSE)
# the ./ at the start of the here function defines where to start
```
