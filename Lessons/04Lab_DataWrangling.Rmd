...
# can also use NTL.phys.data %>% filter(year4==1999)
...
# can also use NTL.phys.data <- %>% 
    # filter (
      # lakename == "Tuesday Lake", (<- can use comma or &)
      # year4 %in% c(1990:1999) [inlcuding the vector notation c() isn't necessary for this to work- but this is only the case for _:_ - it wouldn't work for "year 4 %in% 1990,1999"]
    # )

```
Question: Why don't we filter using row numbers?

> Answer: We don't filter using row numbers because data is sorted in columns.
...
NTL.phys.data.July <- NTL.phys.data %>% 
  filter(
    ...
    # can also use month(sampledate) == 7
...
NTL.phys.data.avg_temp <- NTL.phys.data.July %>% 
  summarize(
    filter(
    depth %in% c(0),
    mutate(mean(NTL.phys.data$temperature_C))))
