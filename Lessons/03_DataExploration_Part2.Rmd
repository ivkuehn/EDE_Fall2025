...
# creating histogram that sorts data into 10 bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

# creating histogram that sorts data into 20 bins
ggplot(USGS.flow.data) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

# sets histogram with 10 bins and an x-axis range from 0-500
ggplot(USGS.flow.data, aes(x = discharge.mean)) +
  geom_histogram(binwidth = 10) + 
  scale_x_continuous(limits = c(0, 500))
  
# creating histogram with the x-axis set to avg gage height
ggplot(USGS.flow.data) +
...
```{r, fig.height = 3, fig.width = 4}
#within the brackets above, it defines the set of the graph space
#
ggplot(USGS.flow.data) +
...
#when the column names change, aes must be applied individually, not within ggplot
