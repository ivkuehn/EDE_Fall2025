...
nutrients.data <- read.csv(file = here("./Data/Processed_KEY/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv"), stringsAsFactors = TRUE)
litter.data <- read.csv(file=here("./Data/Processed_KEY/NEON_NIWO_Litter_mass_trap_Processed.csv"), stringsAsFactors = TRUE)


#2 
nutrients.data$sampledate <- as.Date(nutrients.data$sampledate, format = "%Y-%m-%d")
...
tp_boxplot <- nutrients.data %>% 
  ...
  labs(color = "Lake Name") +
  my_theme + 
  theme(axis.title = element_text(size = 12), legend.position = "top", legend.text = element_text(size = 6), legend.title = element_text(size = 8))
tp_boxplot
...
Litter.plot <- needle.data %>% 
  ...
  xlab("Date") +
  labs(title = "Needle Dry Mass Across Collection Dates", color = "NLCD Class") +
